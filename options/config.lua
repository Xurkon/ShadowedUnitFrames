...
local function setColor(info, r, g, b, a)
	local key = info[#info]
	local color = ShadowUF.db.profile.classColors[key]
	if( not color ) then
		color = {}
		ShadowUF.db.profile.classColors[key] = color
	end
...
-- Determine realm type and set up class colors appropriately
	local realmName = GetRealmName():lower()
	local isAscensionRealm = realmName:match("conquest") or realmName:match("coa") or realmName:match("azeroth")

	-- Build class color options: show ALL classes from RAID_CLASS_COLORS, no hiding
	for classToken in pairs(RAID_CLASS_COLORS) do
		local entry = {}
		for k, v in pairs(Config.classTable) do
			entry[k] = v
		end
		-- Show everything, no hiding
		entry.hidden = function() return false end
		options.args.general.args.color.args.classColors.args[classToken] = entry
	end

	-- PET and VEHICLE always visible
					...
					documentation = {
						...
						args = {
							doc = {
								...
								name = L["Documentation"],
								set = false,
								get = function() return "https://xurkon.github.io/ShadowedUnitFrames/" end,
								width = "full",
							},
						},
					},
